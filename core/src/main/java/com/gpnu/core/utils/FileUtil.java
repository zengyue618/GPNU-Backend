...
public class FileUtil {
    ...
    while (entries.hasMoreElements()) {
      ...
      if (entry.isDirectory()) {
        ...
        try (InputStream src = source.getInputStream(entry)) {
          try (OutputStream output = new BufferedOutputStream(Files.newOutputStream(newFile.toPath()))) {
  ...
  private static void zipFile(String path, File input, ZipOutputStream zout)
    ...
    if (input.isDirectory()) {
      ...
      try (InputStream fileInputStream = new BufferedInputStream(Files.newInputStream(input.toPath()))) {
