...
import com.gpnu.server.var.ParamReference;
import com.gpnu.server.var.VariableManager;
...
public class TaskParamController extends BaseController {
    ...
    public Object createVar(@RequestBody @OperationObj ProjectVar var) {
        ...
        var.setTrash(false);
        var.setCreateTime(Long.parseLong(DateUtil.format(new Date(), "yyMMddHHmm")));
        var.setTeam(ContextUtil.getCurrentUser().getTeam());
        if (var.getInitValue() != null) {
            var.setInitTime(Long.parseLong(DateUtil.format(new Date(), "yyMMddHHmm")));
        }
