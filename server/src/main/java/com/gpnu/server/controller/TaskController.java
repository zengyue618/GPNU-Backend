...
import com.gpnu.server.service.TaskService;
...
public class TaskController extends BaseController {
    ...
    private TaskWorkFlow buildTaskInfo(TaskGraph taskGraph) {
        ...
        taskInfo.setCreateTime(Long.parseLong(DateUtil.format(new Date(), "yyMMddHHmm")));
        ...
        for (TaskGraph.TaskNode taskNode : taskGraph.getNodeList()) {
            ...
            jobInfo.setCreateTime(Long.parseLong(DateUtil.format(new Date(), "yyMMddHHmm")));
            ...
            if (pkg == null) {
                if (jobInfoInDb == null) {
                    throw new GPNUException("pkg and job info not found", ErrorCodes.ERROR_PARAM);
                }
